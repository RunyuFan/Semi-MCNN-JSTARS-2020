...
def main(args):
    ...
    train_datasets = CarDateSet('.\\ShenzhenLC56-6-4\\train_data\\', "./data/trainShenzhenLC56-6-4-teacher.txt", transforms=None)
    test_datasets = CarDateSet('.\\ShenzhenLC56-6-4\\test_data\\', "./data/testShenzhenLC56-6-4-teacher.txt", transforms=None)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc_1 > best_acc_1:
            ...
            torch.save(model1, os.path.join(args.model_path, 'ShenzhenLC-6-4-teacher-ResNet50.pth'))
            best_acc_1 = acc_1
        if acc_2 > best_acc_2:
            print('save new best acc_2', acc_2)
            torch.save(model2, os.path.join(args.model_path, 'ShenzhenLC-6-4-teacher-resnext50_32x4d.pth'))
            best_acc_2 = acc_2
        if acc_3 > best_acc_3:
            print('save new best acc_3', acc_3)
            torch.save(model3, os.path.join(args.model_path, 'ShenzhenLC-6-4-teacher-shufflenetv2_x1.pth'))
            best_acc_3 = acc_3
    print("Model save to %s."%(os.path.join(args.model_path, 'ShenzhenLC-6-4-teacher-model.pth')))
