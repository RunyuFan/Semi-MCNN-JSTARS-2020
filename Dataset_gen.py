...
def select_top_k(k=10):
    sampled_image_dict = {}
    sampled_image_dict["all"] = []
    with open("./sampling_dict_GID.json", "r", encoding="utf-8", errors="ignore") as f:
    ...
    with open("./selected_image_GID.json", "w") as f:
        f.write(j)
...
def main():
    ...
    label_dict = {'bareland': 0, 'cropland': 1, 'forest': 2, 'impervious': 3, 'shrub': 4, 'water': 5}
    lulc1 = LULC('./model/ResNet50-shenzhen-5-5-lulc-6-PreGID.pth')
    lulc2 = LULC('./model/resnext50_32x4d-shenzhen-5-5-lulc-6-PreGID.pth')
    lulc3 = LULC('./model/MSDNet-shenzhen-5-5-lulc-6-PreGID.pth')
    ...
    with open("./sampling_dict_GID.json", "w") as f:
        f.write(j)
...
def gen_data():
    ...
    print('gen data')
    select_top_k(1000)
    with open("./selected_image_GID.json", "r", encoding="utf-8", errors="ignore") as f:
    ...
    for img in image_list:
        ...
        try:
            ...
            if not os.path.exists('.\\GenDataset1000GID'):
                os.mkdir('.\\GenDataset1000GID')
            if not os.path.exists(os.path.join('.\\GenDataset1000GID', label_image)):
                os.mkdir(os.path.join('.\\GenDataset1000GID', label_image))
            print(os.path.join(os.path.join('.\\GenDataset1000GID', label_image), img[0].split('\\')[-1]))
            im.save(os.path.join(os.path.join('.\\GenDataset1000GID', label_image), img[0].split('\\')[-1]))
            # os.remove(img[0])
        except:
            print('image deleted!')
        # im = load_img(img[0])  # , target_size=(224, 224))
