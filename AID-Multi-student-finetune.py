...
def main(args):
    ...
    # train_datasets = CarDateSet('.\\GenDataset300AID\\', './data/trainAID-gen.txt', transforms=None)
    ...
    if args.pretrained:
        ...
        model1 = torch.load('.\\model-AID\\AID-30-student-ResNet50.pth')
        # model2 = ResNet34()
        model2 = torch.load('.\\model-AID\\AID-30-student-resnext50_32x4d.pth')
        model3 = torch.load('.\\model-AID\\AID-30-student-shufflenetv2_x1.pth')
        # print(model3)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc_1 > best_acc_1:
            ...
            torch.save(model1, os.path.join(args.model_path, 'AID-30-student-finetune-ResNet50.pth'))
            best_acc_1 = acc_1
        if acc_2 > best_acc_2:
            print('save new best acc_2', acc_2)
            torch.save(model2, os.path.join(args.model_path, 'AID-30-student-finetune-resnext50_32x4d.pth'))
            best_acc_2 = acc_2
        if acc_3 > best_acc_3:
            print('save new best acc_3', acc_3)
            torch.save(model3, os.path.join(args.model_path, 'AID-30-student-finetune-shufflenetv2_x1.pth'))
            best_acc_3 = acc_3
    print("Model save to %s."%(os.path.join(args.model_path, 'AID-30-student-finetune-model.pth')))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_name", default='AID-30-student-finetune', type=str)
