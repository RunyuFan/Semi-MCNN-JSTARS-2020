...
def main(args):
    ...
    # train_datasets = CarDateSet('.\\GenDataset300ShenzhenLC\\', './data/trainShenzhenLC-gen.txt', transforms=None)
    ...
    # test_datasets = CarDateSet('G:\\LULC\\PytorchLULC\\ShenzhenLC-RESISC45-unlabel-test\\test_data\\', './data/testShenzhenLC.txt', transforms=None)
    ...
    if args.pretrained:
        ...
        # model3.fc = nn.Linear(channel_in_3, args.num_class)
            # lulc1 = LULC('.\\model-ShenzhenLC\\ShenzhenLC-45-teacher-ResNet50.pth')
            # lulc2 = LULC('.\\model-ShenzhenLC\\ShenzhenLC-45-teacher-resnext50_32x4d.pth')
            # lulc3 = LULC('.\\model-ShenzhenLC\\ShenzhenLC-45-teacher-shufflenetv2_x1.pth')
        model1 = torch.load('.\\model-ShenzhenLC\\ShenzhenLC-6-student-ResNet50.pth')
        # model2 = ResNet34()
        model2 = torch.load('.\\model-ShenzhenLC\\ShenzhenLC-6-student-resnext50_32x4d.pth')
        model3 = torch.load('.\\model-ShenzhenLC\\ShenzhenLC-6-student-shufflenetv2_x1.pth')
        # print(model3)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc_1 > best_acc_1:
            ...
            torch.save(model1, os.path.join(args.model_path, 'ShenzhenLC-6-student-finetune-ResNet50-3.pth'))
            best_acc_1 = acc_1
        if acc_2 > best_acc_2:
            print('save new best acc_2', acc_2)
            torch.save(model2, os.path.join(args.model_path, 'ShenzhenLC-6-student-finetune-resnext50_32x4d-3.pth'))
            best_acc_2 = acc_2
        if acc_3 > best_acc_3:
            print('save new best acc_3', acc_3)
            torch.save(model3, os.path.join(args.model_path, 'ShenzhenLC-6-student-finetune-shufflenetv2_x1-3.pth'))
            best_acc_3 = acc_3
    print("Model save to %s."%(os.path.join(args.model_path, 'ShenzhenLC-6-student-finetune-model-3.pth')))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_name", default='ShenzhenLC-6-student-finetune', type=str)
