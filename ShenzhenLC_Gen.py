...
def select_top_k(acc=.9):
    ...
    k = 1000000
    with open("./sampling_dict_ShenzhenLC.json", "r", encoding="utf-8", errors="ignore") as f:
    ...
    with open("./selected_image_ShenzhenLC.json", "w") as f:
        f.write(j)
...
test_image_path = '.\\ShenzhenLC\\unlabel_data\\'
...
def main():
    ...
    with open("./sampling_dict_ShenzhenLC.json", "w") as f:
        f.write(j)
...
def gen_data():
    ...
    print('gen data')
    select_top_k(0.797)
    with open("./selected_image_ShenzhenLC.json", "r", encoding="utf-8", errors="ignore") as f:
    ...
    for img in image_list:
        ...
        try:
            ...
            if not os.path.exists('.\\GenDatasetShenzhenLC'):
                os.mkdir('.\\GenDatasetShenzhenLC')
            if not os.path.exists(os.path.join('.\\GenDatasetShenzhenLC', label_image)):
                os.mkdir(os.path.join('.\\GenDatasetShenzhenLC', label_image))
            print(os.path.join(os.path.join('.\\GenDatasetShenzhenLC', label_image), img[0].split('\\')[-1]))
            im.save(os.path.join(os.path.join('.\\GenDatasetShenzhenLC', label_image), img[0].split('\\')[-1]))
            # os.remove(img[0])
        except:
            print('image deleted!')
        # im = load_img(img[0])  # , target_size=(224, 224))
