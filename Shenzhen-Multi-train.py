...
from dataset_ShenzhenLC import CarDateSet
# from resnet_lulc_ShenzhenLC import ResNet18, ResNet34, ResNet50, ResNet101
import argparse
# from ResNext_ShenzhenLC import resnext50_32x4d
# from MSDnet_ShenzhenLC import msdnet
...
def main(args):
    ...
    train_datasets = CarDateSet('.\\ShenzhenLC\\train_data\\', './data/trainShenzhenLC-teacher.txt', transforms=None)
    test_datasets = CarDateSet('.\\ShenzhenLC\\test_data\\', './data/testShenzhenLC-teacher.txt', transforms=None)
    # test_datasets = CarDateSet('G:\\LULC\\PytorchLULC\\ShenzhenLC-RESISC6-unlabel-test\\test_data\\', './data/testShenzhenLC.txt', transforms=None)
    ...
    if args.pretrained:
        ...
        model1.load_state_dict(torch.load('.\\ImageNet-models\\resnet50-19c8e357.pth'))
        ...
        model2.load_state_dict(torch.load('.\\ImageNet-models\\resnext50_32x4d-7cdf4587.pth'))
        ...
        model3.load_state_dict(torch.load('.\\ImageNet-models\\shufflenetv2_x1-5666bf0f80.pth'))
        channel_in_3 = model3.fc.in_features  # 获取fc层的输入通道数
        # 然后把resnet的fc层替换成自己分类类别的fc层
        model3.fc = nn.Linear(channel_in_3, args.num_class)
            # lulc1 = LULC('.\\model-ShenzhenLC\\ShenzhenLC-6-teacher-ResNet50.pth')
            # lulc2 = LULC('.\\model-ShenzhenLC\\ShenzhenLC-6-teacher-resnext50_32x4d.pth')
            # lulc3 = LULC('.\\model-ShenzhenLC\\ShenzhenLC-6-teacher-shufflenetv2_x1.pth')
        # model1 = torch.load('.\\model-ShenzhenLC\\ShenzhenLC-6-teacher-ResNet50.pth')
        # # model2 = ResNet34()
        # model2 = torch.load('.\\model-ShenzhenLC\\ShenzhenLC-6-teacher-resnext50_32x4d.pth')
        # model3 = torch.load('.\\model-ShenzhenLC\\ShenzhenLC-6-teacher-shufflenetv2_x1.pth')
        # print(model3)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc_1 > best_acc_1:
            ...
            torch.save(model1, os.path.join(args.model_path, 'ShenzhenLC-6-teacher-ResNet50-timetest.pth'))
            best_acc_1 = acc_1
        # if acc_2 > best_acc_2:
        #     print('save new best acc_2', acc_2)
        #     torch.save(model2, os.path.join(args.model_path, 'ShenzhenLC-6-teacher-resnext50_32x4d-timetest.pth'))
        #     best_acc_2 = acc_2
        # if acc_3 > best_acc_3:
        #     print('save new best acc_3', acc_3)
        #     torch.save(model3, os.path.join(args.model_path, 'ShenzhenLC-6-teacher-shufflenetv2_x1-timetest.pth'))
        #     best_acc_3 = acc_3

    print("Model save to %s."%(os.path.join(args.model_path, 'ShenzhenLC-6-teacher-model-nopretrain.pth')))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_name", default='ShenzhenLC-6-student', type=str)
    parser.add_argument("--model_path", default='./model-ShenzhenLC', type=str)
