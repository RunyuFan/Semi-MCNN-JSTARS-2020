...
def test_image_predict_top_k(lulc1, lulc2, lulc3, test_image_path):
    ...
    image_list_ori = generator_list_of_imagepath(test_image_path)
    print(len(image_list_ori))
    image_list = random.sample(image_list_ori, 10000)
...
def select_top_k(acc=.9):
    ...
    k = 1000000
    with open("./sampling_dict_ShenzhenLC_Real.json", "r", encoding="utf-8", errors="ignore") as f:
    ...
    with open("./selected_image_ShenzhenLC_Real.json", "w") as f:
        f.write(j)
...
test_image_path = 'G:\\LULC\\unlabel\\'
...
def generator_list_of_imagepath2(test_image_path):
    image_list = []
...
def main():
    ...
    with open("./sampling_dict_ShenzhenLC_Real.json", "w") as f:
        f.write(j)
...
def gen_data():
    ...
    print('gen data')
    select_top_k(0.749)
    with open("./selected_image_ShenzhenLC_Real.json", "r", encoding="utf-8", errors="ignore") as f:
    ...
    for img in image_list:
        ...
        try:
            ...
            if not os.path.exists('.\\GenDatasetShenzhenLCReal'):
                os.mkdir('.\\GenDatasetShenzhenLCReal')
            if not os.path.exists(os.path.join('.\\GenDatasetShenzhenLCReal', label_image)):
                os.mkdir(os.path.join('.\\GenDatasetShenzhenLCReal', label_image))
            print(os.path.join(os.path.join('.\\GenDatasetShenzhenLCReal', label_image), img[0].split('\\')[-1]))
            im.save(os.path.join(os.path.join('.\\GenDatasetShenzhenLCReal', label_image), img[0].split('\\')[-1]))
            # os.remove(img[0])
        except:
            print('image deleted!')
        # im = load_img(img[0])  # , target_size=(224, 224))
