...
def main(args):
    ...
    train_datasets = CarDateSet('.\\GenDatasetNWPU\\', './data/trainNWPU-gen.txt', transforms=None)
    ...
    if args.pretrained:
        ...
        model1 = torch.load('.\\model-NWPU\\NWPU-45-teacher-ResNet50-1.pth')
        # model2 = ResNet34()
        model2 = torch.load('.\\model-NWPU\\NWPU-45-teacher-resnext50_32x4d-1.pth')
        model3 = torch.load('.\\model-NWPU\\NWPU-45-teacher-shufflenetv2_x1-1.pth')
        # print(model3)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc_1 > best_acc_1:
            ...
            torch.save(model1, os.path.join(args.model_path, 'NWPU-45-student-ResNet50-1.pth'))
            best_acc_1 = acc_1
        if acc_2 > best_acc_2:
            print('save new best acc_2', acc_2)
            torch.save(model2, os.path.join(args.model_path, 'NWPU-45-student-resnext50_32x4d-1.pth'))
            best_acc_2 = acc_2
        if acc_3 > best_acc_3:
            print('save new best acc_3', acc_3)
            torch.save(model3, os.path.join(args.model_path, 'NWPU-45-student-shufflenetv2_x1-1.pth'))
            best_acc_3 = acc_3
    print("Model save to %s."%(os.path.join(args.model_path, 'NWPU-45-student-model-1.pth')))
