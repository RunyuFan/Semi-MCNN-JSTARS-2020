...
def draw(y_true,y_pred,labels):
    ...
    for x_val, y_val in zip(x.flatten(), y.flatten()):
        ...
        if c > 0.01:
            plt.text(x_val, y_val, "%0.2f" % (c,), color='red', fontsize=70, va='center', ha='center') # ) # ,fontfamily='Times New Roman') 50
    # offset the tick
    ...
    plt.savefig('NWPU-Semi-MCNN-final.png', format='png')
    plt.close()
...
def main(args):
    ...
    if args.pretrained:
        ...
        model1 = torch.load('.\\model-NWPU\\NWPU-45-student-finetune-ResNet50-1.pth')
        # model2 = ResNet34()
        model2 = torch.load('.\\model-NWPU\\NWPU-45-student-finetune-resnext50_32x4d-1.pth')
        model3 = torch.load('.\\model-NWPU\\NWPU-45-student-finetune-shufflenetv2_x1-1.pth')
