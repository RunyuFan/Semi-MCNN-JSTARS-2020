...
class MultiModalNet(nn.Module):
    def __init__(self, backbone1, backbone2, backbone3):
        super().__init__()
        self.model1 = backbone1
        self.model2 = backbone2
        self.model3 = backbone3
        self.fc = nn.Linear(45, 45)
    def forward(self, x):

        model1 = self.model1(x)
        model2 = self.model2(x)
        model3 = self.model3(x)

        x_sum = torch.add(model1, model2)
        x_sum = torch.add(x_sum, model3)
        # x_sum = self.fc(x_sum)
        output = self.fc(x_sum)
        return output
...
def main(args):
    ...
    if args.pretrained:
        ...
        model1 = torch.load('.\\model-NWPU\\NWPU-45-student-ResNet50.pth')
        for param in model1.parameters():
            param.requires_grad = False
        channel_in_1 = model1.fc.in_features  # 获取fc层的输入通道数
        # 然后把resnet的fc层替换成自己分类类别的fc层
        model1.fc = nn.Linear(channel_in_1, args.num_class)
        for param in model1.fc.parameters():
            param.requires_grad = True

        # model2 = ResNet34()
        model2 = torch.load('.\\model-NWPU\\NWPU-45-student-resnext50_32x4d.pth')
        ...
        for param in model2.fc.parameters():
            param.requires_grad = True
        model3 = torch.load('.\\model-NWPU\\NWPU-45-student-shufflenetv2_x1.pth')
    ...
    model_sum = MultiModalNet(model1, model2, model3)
    print('model_sum parameters:', sum(p.numel() for p in model_sum.parameters() if p.requires_grad))

    model1 = model_sum.to(device)
    ...
    # # Optimization
    ignored_params = list(map(id, model1.fc.parameters()))
    base_params = filter(lambda p: id(p) not in ignored_params, model1.parameters())

    # 对不同参数设置不同的学习率
    params_list = [{'params': base_params, 'lr': 1e-5}, ]
    params_list.append({'params': model1.fc.parameters(), 'lr': 1e-3})

    # optimizer1 = optim.Adam(model1.parameters(), lr=args.lr, weight_decay=1e-6)
    optimizer1 = optim.Adam(params_list, lr=args.lr, weight_decay=1e-6)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc_1 > best_acc_1:
            ...
            torch.save(model1, os.path.join(args.model_path, 'NWPU-45-student-finetune-Ensemble-fix.pth'))
            best_acc_1 = acc_1
        # if acc_2 > best_acc_2:
        #     print('save new best acc_2', acc_2)
        #     torch.save(model2, os.path.join(args.model_path, 'NWPU-45-student-finetune-resnext50_32x4d-fix.pth'))
        #     best_acc_2 = acc_2
        # if acc_3 > best_acc_3:
        #     print('save new best acc_3', acc_3)
        #     torch.save(model3, os.path.join(args.model_path, 'NWPU-45-student-finetune-shufflenetv2_x1-fix.pth'))
        #     best_acc_3 = acc_3
