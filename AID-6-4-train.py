...
def main(args):
    ...
    train_datasets = CarDateSet('.\\AID-6-4\\train_data\\', './data/trainAID-6-4.txt', transforms=None)
    test_datasets = CarDateSet('.\\AID-6-4\\test_data\\', './data/testAID-6-4.txt', transforms=None)
    ...
    if args.pretrained:
        ...
        model3.fc = nn.Linear(channel_in_3, args.num_class)
            # lulc1 = LULC('.\\model-AID\\AID-6-teacher-ResNet50.pth')
            # lulc2 = LULC('.\\model-AID\\AID-6-teacher-resnext50_32x4d.pth')
            # lulc3 = LULC('.\\model-AID\\AID-6-teacher-shufflenetv2_x1.pth')
        # model1 = torch.load('.\\model-AID\\AID-6-teacher-ResNet50.pth')
        # # model2 = ResNet34()
        # model2 = torch.load('.\\model-AID\\AID-6-teacher-resnext50_32x4d.pth')
        # model3 = torch.load('.\\model-AID\\AID-6-teacher-shufflenetv2_x1.pth')
        # print(model3)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc_1 > best_acc_1:
            ...
            torch.save(model1, os.path.join(args.model_path, 'AID-30-teacher-6-4-ResNet50.pth'))
            best_acc_1 = acc_1
        if acc_2 > best_acc_2:
            print('save new best acc_2', acc_2)
            torch.save(model2, os.path.join(args.model_path, 'AID-30-teacher-6-4-resnext50_32x4d.pth'))
            best_acc_2 = acc_2
        if acc_3 > best_acc_3:
            print('save new best acc_3', acc_3)
            torch.save(model3, os.path.join(args.model_path, 'AID-30-teacher-6-4-shufflenetv2_x1.pth'))
            best_acc_3 = acc_3
    print("Model save to %s."%(os.path.join(args.model_path, 'AID-30-teacher-6-4-model.pth')))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_name", default='AID-30-student', type=str)
