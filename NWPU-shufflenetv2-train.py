...
from torchvision.models import resnet50, resnext50_32x4d, densenet121, shufflenet_v2_x1_0
...
def main(args):
    ...
    if args.pretrained:
        ...
        model3.load_state_dict(torch.load(args.pretrained_model))
        # print(model3)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 1 == 0:
            ...
            # print('----------------------------------------------------')
            # for i in range(args.num_class):
            #     print('Model shufflenetv2_x1 - Accuracy of %5s : %2d %%: Correct Num: %d in Total Num: %d' % (
        ...
        #     torch.save(model1, os.path.join(args.model_path, 'NWPU-45-teacher-ResNet50-%s.pth' % (args.model_name)))
        #     best_acc_1 = acc_1
        # if acc_2 > best_acc_2:
        #     print('save new best acc_2', acc_2)
        #     torch.save(model2, os.path.join(args.model_path, 'NWPU-45-teacher-resnext50_32x4d-%s.pth' % (args.model_name)))
        #     best_acc_2 = acc_2
        if acc_3 > best_acc_3:
            print('save new best acc_3', acc_3)
            torch.save(model3, os.path.join(args.model_path, 'NWPU-45-teacher-shufflenetv2_x1-1.pth'))
            best_acc_3 = acc_3
    print("Model save to %s."%(os.path.join(args.model_path, 'NWPU-45-teacher-model-%s.pth' % (args.model_name))))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_name", default='NWPU-45-teacher', type=str)
    parser.add_argument("--model_path", default='./model-NWPU', type=str)
    parser.add_argument("--pretrained", default=False, type=bool)
    parser.add_argument("--pretrained_model", default='./ImageNet-models/shufflenetv2_x1-5666bf0f80.pth', type=str)
